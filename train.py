    ...
    def train(self):
        ...
        for iter in range(self.global_step + 1, tt.arg.train_iteration + 1):
            ...
            [support_data,
             support_label,
             query_data,
             query_label] = self.data_loader['train'].get_task_batch()
            
            support_data,support_label,query_data,query_label = support_data.to(tt.arg.device),support_label.to(tt.arg.device),query_data.to(tt.arg.device),query_label.to(tt.arg.device)
            # set as single data
            full_data = torch.cat([support_data, query_data], 1).to(tt.arg.device)
            full_label = torch.cat([support_label, query_label], 1).to(tt.arg.device)
            ...
            init = init_edge.clone()
            
            ones_vec = torch.ones_like(init[:, 0, :, :]).to(tt.arg.device)
            negones_vec = -1 * torch.ones_like(init[:, 0, :, :]).to(tt.arg.device)
            zero_vec = torch.zeros_like(init[:, 0, :, :]).to(tt.arg.device)

            e0 = torch.where(init[:, 0, :, :] > 0.5, ones_vec, init[:, 0, :, :])
            e0 = torch.where(init[:, 0, :, :] == 0.5, zero_vec, e0)
            e0 = torch.where(init[:, 0, :, :] < 0.5, negones_vec, e0)
            diag_mask = 1.0 - torch.eye(full_data.size(1)).unsqueeze(0).repeat(full_data.size(0), 1, 1).to(tt.arg.device)
            e0 = e0 * diag_mask
            ...
            full_data = torch.stack(full_data, dim=1).to(tt.arg.device) # batch_size x num_samples x featdim
            ...
            query_node_pred_layers = [torch.bmm(full_logit_layer[:, 0, num_supports:, :num_supports], self.one_hot_encode(tt.arg.num_ways_train, support_label.long()).float()) for full_logit_layer in full_logit_layers] # (num_tasks x num_quries x num_supports) * (num_tasks x num_supports x num_ways)
            ...
            if test_ac > self.test_acc:
                self.test_acc = test_ac
    ...
    def eval(self, partition='test', log_flag=True):
        ...
        for iter in range(tt.arg.test_iteration//tt.arg.test_batch_size):
            ...
            [support_data,
             support_label,
             query_data,
             query_label] = self.data_loader[partition].get_task_batch()
...
if __name__ == '__main__':
    ...
    tt.arg.alpha = 0.99 if tt.arg.num_shots == 1 else 0.1
    ...
    tt.arg.num_edge_features = 128
    tt.arg.num_node_features = 128
    tt.arg.emb_size = 128
    ...
    if tt.arg.dataset == 'mini':
        train_loader = DataLoader(dataset=MiniImagenet(root=tt.arg.dataset_root, partition='train'),
                              num_tasks=tt.arg.meta_batch_size,
                              num_ways=tt.arg.num_ways,
                              num_shots=tt.arg.num_shots,
                              num_queries=1,
                              epoch_size=tt.arg.train_iteration)
        valid_loader = DataLoader(dataset=MiniImagenet(root=tt.arg.dataset_root, partition='val'),
                              num_tasks=tt.arg.test_batch_size,
                              num_ways=tt.arg.num_ways_test,
                              num_shots=tt.arg.num_shots_test,
                              num_queries=1,
                              epoch_size=tt.arg.test_iteration)
        test_loader = DataLoader(dataset=MiniImagenet(root=tt.arg.dataset_root, partition='test'),
                             num_tasks=tt.arg.test_batch_size,
                             num_ways=tt.arg.num_ways_test,
                             num_shots=tt.arg.num_shots_test,
                             num_queries=1,
